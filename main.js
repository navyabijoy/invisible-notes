...
function getImagesDir() {
  if (!imagesDir) imagesDir = path.join(app.getPath("userData"), "note-images");
  return imagesDir;
}

// Filenames are always generated by us (`<note id>-<time>.<ext>`). Reject
// anything else — especially path separators or ".." — before it reaches fs.
const SAFE_IMAGE_NAME = /^[\w-]+\.[a-z0-9]+$/;

// Every image filename a record references: the persisted list plus any name
// still present in the body markup (older records wrote only one of the two).
function referencedImageNames(record) {
  const names = new Set();
  for (const name of (record && record.images) || []) {
    if (SAFE_IMAGE_NAME.test(name)) names.add(name);
  }
  for (const m of ((record && record.text) || "").matchAll(
    /data-name="([^"]+)"/g,
  )) {
    if (SAFE_IMAGE_NAME.test(m[1])) names.add(m[1]);
  }
  return names;
}

function deleteImageFiles(record) {
  for (const name of referencedImageNames(record)) {
    try {
      fs.unlinkSync(path.join(getImagesDir(), name));
    } catch (_) {}
  }
}
...
ipcMain.on("note:update", (e, payload) => {
  ...
  const { id, text, rich, images, color, opacity, fontSize, monospace, ghost } =
    payload;
  const patch = {};
  if (typeof text === "string") patch.text = text;
  if (typeof rich === "boolean") patch.rich = rich;
  if (Array.isArray(images)) {
    patch.images = images.filter(
      (name) => typeof name === "string" && SAFE_IMAGE_NAME.test(name),
    );
  }
...
// ---------- Pasted images ----------
ipcMain.handle("note:imagesDir", () => getImagesDir());
...
ipcMain.handle("note:saveImage", async (e, { id, type, bytes }) => {
  const record = store.get(id);
  if (!record) return null;
  if (!Object.prototype.hasOwnProperty.call(IMAGE_EXT, type)) return null;
  if (!(bytes instanceof Uint8Array) || bytes.length === 0) return null;
  const ext = IMAGE_EXT[type];
  const name = `${id}-${Date.now().toString(36)}-${Math.random()
    .toString(36)
    .slice(2, 6)}.${ext}`;
  try {
    // Async I/O: a multi-megabyte screenshot must not block the main process
    // event loop (it would freeze every note window during paste).
    await fs.promises.mkdir(getImagesDir(), { recursive: true });
    await fs.promises.writeFile(
      path.join(getImagesDir(), name),
      Buffer.from(bytes),
    );
  } catch (err) {
...
// Images removed from a note keep their file for the rest of the session so
// in-editor undo still works; orphans are swept on the next launch instead.
function pruneOrphanImages() {
  let names;
  try {
    names = fs.readdirSync(getImagesDir());
  } catch (_) {
    return;
  }
  const referenced = new Set();
  for (const record of store.all()) {
    for (const name of referencedImageNames(record)) referenced.add(name);
  }
