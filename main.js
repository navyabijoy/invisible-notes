...
// Filenames are always generated by us (`<note id>-<timestamp>.<ext>`).
// Anything else in a record — especially path separators or ".." — is a
// corrupted/tampered value and must never reach an fs call.
...
ipcMain.handle('note:saveImage', async (e, { id, type, bytes }) => {
  const record = store.get(id);
  ...
  try {
    // Async I/O: a multi-megabyte screenshot must not block the main
    // process event loop (it would freeze every note window during paste).
    await fs.promises.mkdir(imagesDir, { recursive: true });
    await fs.promises.writeFile(path.join(imagesDir, name), Buffer.from(bytes));
  } catch (err) {
